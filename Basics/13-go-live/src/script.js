...
const parameters = {}
parameters.text = 'Three.js Journey'
parameters.numberOfInstances = 1000
parameters.numberOfRows = 10
parameters.numberOfLayers = 10
parameters.spacingBetweenInstances = 3
...
// const matcapTexture = textureLoader.load('/static/matcapTexture.png')
...
fontLoader.load('/font/gentilis_regular.typeface.json', (font) => {
    ...
    const xOffset =
        ((numberOfInstancesPerRowPerLayer - 1) *
            parameters.spacingBetweenInstances) /
        2
    const yOffset =
        ((parameters.numberOfLayers - 1) * parameters.spacingBetweenInstances) /
        2
    const zOffset =
        ((parameters.numberOfRows - 1) * parameters.spacingBetweenInstances) / 2
    ...
    for (
        let layerIndex = 0;
        layerIndex < parameters.numberOfLayers;
        layerIndex++
    ) {
        for (let rowIndex = 0; rowIndex < parameters.numberOfRows; rowIndex++) {
            for (
                let instanceIndex = 0;
                ...
                const xPosition =
                    instanceIndex * parameters.spacingBetweenInstances - xOffset
                const yPosition =
                    rowIndex * parameters.spacingBetweenInstances - yOffset
                const zPosition =
                    layerIndex * parameters.spacingBetweenInstances - zOffset
    ...
    text.instanceColor.needsUpdate = true
    text.instanceMatrix.needsUpdate = true
    scene.add(text)

    textFolder.addBinding(parameters, 'text').on('change', (event) => {
        text.geometry.dispose()
        parameters.text = event.value
...
const tick = () => {
    ...
    if (
        positionArray.length === parameters.numberOfInstances &&
        text !== null
    ) {
        for (let i = 0; i < parameters.numberOfInstances; i++) {
            dummy.position.set(
                positionArray[i].x,
                positionArray[i].y,
                positionArray[i].z
